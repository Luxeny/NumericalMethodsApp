...
namespace DehotomiaM
{
    class Matrix
    {
        ...
        private Matrix getMinor(int row, int column)
        {
            ...
            double[,] minor = new double[Row - 1, Col - 1]; //создаёт мновую матрица
            for (int i = 0; i < this.Row; i++) // перебор всех элементов матрицы 
            {
            ...
            return new Matrix(minor); //ищем минор матрицы по заданным стр
        }
        ...
        public static double Determ(Matrix m)
        {
            ...
            if (length == 1) det = m.Args[0, 0];// определитель еденственному элементу матрицы
            if (length == 2) det = m.Args[0, 0] * m.Args[1, 1] - m.Args[0, 1] * m.Args[1, 0]; //если двум, то определитель находится по этой формуле

            if (length > 2) //перебор всех элементов первой строки. Для каждого определителя вычисляется определитель минора, путем исключения первой
                //строки и текущего столбца. Определяем знак , а потом произведния текущего элемента первой строки на опред. минора 
        ...
        public Matrix MinorMatrix() //алгебраические дополнения. 
        {
        ...
        public Matrix InverseMatrix()
        {
            ...
            double k = 1 / Determ(this); //   1/определелитель 

            Matrix minorMatrix = this.MinorMatrix();

            return minorMatrix * k; //алгебраические дополнения делим на K 
        }
    }
}
